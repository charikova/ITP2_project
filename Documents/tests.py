...
class Delivery2(TestCase):
    ...
    def test_TC7(self):
        ...
        p1_have_overdue_on_b1_in_21_days = abs((temp.date - temp.returning_date).days) == 27

        temp = self.p1.documentcopy_set.filter(doc=self.b2)[0]
        p1_have_overdue_on_b2_in_21_days = abs((datetime.datetime.today() - temp.returning_date).days) == 27

        temp = self.p1.documentcopy_set.filter(doc=self.av1)[0]
        p1_have_overdue_on_av1_in_14_days = abs((datetime.datetime.now() - temp.returning_date).days) == 21

        temp = self.p2.documentcopy_set.filter(doc=self.b1)[0]
        p2_have_overdue_on_b1_in_21_days = abs((datetime.datetime.now() - temp.returning_date).days) == 21

        temp = self.p2.documentcopy_set.filter(doc=self.b2)[0]
        p2_have_overdue_on_b2_in_21_days = abs((datetime.datetime.now() - temp.returning_date).days) == 21

        temp = self.p2.documentcopy_set.filter(doc=self.av2)[0]
        p2_have_overdue_on_av2_in_14_days = abs((datetime.datetime.now() - temp.returning_date).days) == 14

        print(temp.returning_date)
        print(temp.date)
        print(datetime.datetime.now())
        print(temp.returning_date - datetime.datetime.now())
